...
TARBİL (Tarımsal İzleme ve Bilgi Sistemi) kapsamında Türkiye genelinde toplanan buğday/arpa saha ölçüm verisinin işlenmesi ve istatistiksel analizi. İTÜ UYSM'de yarı zamanlı çalışırken (Aralık 2025 - Haziran 2026) başlanıp zamanla genişletilen, kurum onayıyla public paylaşılan bir çalışma.

**Ne yapıldı:** 291 istasyon dosyasıyla başlayan ilk aşamada, otomatik bir ETL pipeline'ı kuruldu; bu süreçte gerçek 2 hesaplama hatası bulunup düzeltildi. Devam eden çalışmada veri seti ~800 ham ölçüme (79 il) genişletildi, ardından ~90 versiyonluk iteratif bir temizlik sürecinden geçirilip 617 analiz-hazır kayda (42 il) indirgendi; her bir kaybın nedeni tek tek belgelendi. Aykırı değer tespiti tek bir eşik yerine, akademik literatüre dayanan (Tukey 1977, Iglewicz & Hoaglin 1993) çok aşamalı bir yöntemle (seri-içi MAD, global IQR, global MAD) yapıldı.
...
├── 01_ilk_pipeline/          Stage 1: the initial ETL (291 files -> 239 clean records)
│   ├── scripts/                Extraction, validation, geocoding, visualization
│   └── docs/                   Excel layout reference, data-issue log
└── 02_genisletilmis_analiz/  Stage 2: the expanded pipeline and analysis (-> 617 records, 42 provinces)
